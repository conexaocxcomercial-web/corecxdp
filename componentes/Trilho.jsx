...
export function Trilho({ empresa }) {
  ...
  return (
    <>
      ...
      <aside className="sobre-escuro fixed inset-y-0 left-0 z-30 hidden w-[232px] flex-col bg-grafite lg:flex">
            ...
            return (
              <Link
                ...
                className={`relative flex h-10 items-center rounded-md pl-4 pr-3 text-[13.5px] transition-colors ${
                  ativa
                    ? 'bg-folha/[0.07] font-bold text-folha'
                    : 'text-folha/60 hover:text-folha'
                }`}
              >
                {ativa ? (
                  <span className="absolute left-0 top-1/2 h-5 w-[3px] -translate-y-1/2 rounded-full bg-violeta" />
                ) : null}
                {secao.nome}
              </Link>
            );
        ...
        <div className="px-6 pb-7 pt-6">
          <AssinaturaCX className="h-[22px] w-auto opacity-70" />
        </div>
      </aside>

      {/* Mobile: barra superior com as abas do livro. */}
      <header className="sobre-escuro sticky top-0 z-30 bg-grafite lg:hidden">
            ...
            return (
              <Link
                ...
                className={`relative whitespace-nowrap px-3 pb-2.5 pt-1 text-[13px] ${
                  ativa ? 'font-bold text-folha' : 'text-folha/55'
                }`}
              >
                {secao.nome}
                {ativa ? (
                  <span className="absolute inset-x-3 bottom-0 h-[2px] rounded-full bg-violeta" />
                ) : null}
              </Link>
            );
          })}
